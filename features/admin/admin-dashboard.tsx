...
export function AdminDashboard({ initialProducts }: { initialProducts: Product[] }) {
  ...
  async function refresh() {
    ...
    if (response.ok) {
      ...
      setData(next);
      setProduct((current) => next.products.find((item) => item.id === current.id) ?? current);
    }
  }
  ...
  async function saveVariant(event: FormEvent<HTMLFormElement>) {
    ...
    await post({ action: "variant", variant: {
  ...
  return (
    <div className="admin-shell">
      <aside className="admin-nav"><Link className="brand" href="/"><span className="brand__mark">◐</span><span><strong>Lorem Ipsum</strong><small>Dolor sit amet.</small></span></Link><nav><a href="#overview"><BarChart3 aria-hidden="true" /> Lorem ipsum</a><a href="#products"><Boxes aria-hidden="true" /> Dolor sit</a><a href="#variants"><Boxes aria-hidden="true" /> Amet lorem</a><a href="#orders"><PackageCheck aria-hidden="true" /> Ipsum dolor</a><a href="#fulfillment"><Truck aria-hidden="true" /> Amet elit</a><a href="#reviews"><Star aria-hidden="true" /> Consectetur</a><a href="#campaigns"><Megaphone aria-hidden="true" /> Adipiscing</a><a href="#coupons"><TicketPercent aria-hidden="true" /> Incididunt</a></nav></aside>
      <main className="admin-main">
        ...
        <section className="admin-panel" id="variants"><div className="admin-panel__heading"><div><span className="eyebrow">Lorem ipsum</span><h2>Amet dolor sit.</h2></div><span>{product.variants?.length ?? 0} lorem</span></div><div className="admin-variant-list">{product.variants?.length ? product.variants.map((variant) => <article key={variant.id}><span><strong>{variant.label}</strong><small>{variant.sku}</small></span><b>{variant.price === null ? formatPrice(product.price) : formatPrice(variant.price)}</b><em>{variant.stock} lorem</em></article>) : <p>Lorem ipsum dolor sit amet.</p>}</div><form className="admin-form" key={product.id} onSubmit={saveVariant}><label><span>id</span><Input name="id" defaultValue={`${product.id}-lorem`} required /></label><label><span>sku</span><Input name="sku" defaultValue={`${product.sku}-NEW`} required /></label><label><span>label</span><Input name="label" defaultValue="Lorem 42" required /></label><label><span>price</span><Input name="price" type="number" min="0" placeholder={String(product.price)} /></label><label><span>stock</span><Input name="stock" type="number" min="0" defaultValue="6" required /></label><Button type="submit">Lorem ipsum</Button></form></section>
