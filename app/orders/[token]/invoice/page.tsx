...
export default async function InvoicePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="invoice-page">
      ...
      <div className="invoice-lines"><div className="invoice-row invoice-row--head"><span>Lorem ipsum</span><span>Dolor</span><span>Amet</span><span>Consectetur</span></div>{result.items.map((item) => { const line = item as { sku: string; variant_sku: string | null; variant_label: string | null; name: string; unit_price: number; quantity: number }; return <div className="invoice-row" key={`${line.variant_sku ?? line.sku}-${line.quantity}`}><span><strong>{line.name}</strong><small>{line.variant_sku ?? line.sku}{line.variant_label ? ` · ${line.variant_label}` : ""}</small></span><span>{line.quantity}</span><span>{formatPrice(line.unit_price)}</span><span>{formatPrice(line.unit_price * line.quantity)}</span></div>; })}</div>
